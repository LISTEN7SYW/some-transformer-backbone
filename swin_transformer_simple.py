# Code from : https://github.com/yyvhang/Vision_Transformer_model/blob/main/Swin_model.py
import torch
...
def generate_mask(input_res, window_size, shift_size):
    ...
    for h in h_slice:
        for w in w_slice:
            image_mask[:, h, w, :] = cnt
            cnt += 1
    mask_window = window_partition(image_mask, window_size) #[B*num_window, window_size, window_size, C]
    mask_window = mask_window.reshape(-1, window_size*window_size) #[B*num_window*C, window_size*window_size]

    attn_mask = mask_window.unsqueeze(1) - mask_window.unsqueeze(2) #[B*num_window*C, window_size * window_size, window_size * window_size]
...
class Patch_Embeding(nn.Module):
    ...
    def forward(self, x):
        x = self.patch(x) #[B, C, H, W] , C = dim  H=H/4, W=W/4
